...
def getProducts():
    return [
        {
            'id': 0,
            'title': u'Istio服务网格技术解析与实践',
            'descriptionHtml': u'<a href="https://baike.baidu.com/item/Istio%E6%9C%8D%E5%8A%A1%E7%BD%91%E6%A0%BC%E6%8A%80%E6%9C%AF%E8%A7%A3%E6%9E%90%E4%B8%8E%E5%AE%9E%E8%B7%B5">百度百科</a> Istio是一个开源项目，拥有一个充满活力、开放和多元化的社区，它的目标是赋能开发人员和运维人员，使他们在所有环境中都能敏捷地发布和维护微服务，拥有底层网络的完全可见性，且获得一致的控制和安全能力。在本书中，我们将展示如何利用Istio的功能在云原生世界中运行微服务应用。'
        }
    ]
...
def getProductDetails(product_id, headers):
    ...
    if res and res.status_code == 200:
        ...
        return status, {'error': u'抱歉, 无法提供该商品的详细信息.'}
...
def getProductReviews(product_id, headers):
    ...
    for _ in range(2):
        ...
        if res and res.status_code == 200:
            return 200, res.json()
    status = res.status_code if res is not None and res.status_code else 500
    return status, {'error': u'抱歉, 无法提供该商品的评论信息.'}
...
def getProductRatings(product_id, headers):
    ...
    if res and res.status_code == 200:
        ...
        status = res.status_code if res is not None and res.status_code else 500
        return status, {'error': u'抱歉, 无法提供该商品的评分信息.'}
